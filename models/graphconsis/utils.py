...
def matrix_to_adjlist(M, pad=True):
    adjlist = []
    import numpy
    # Convert general matrix to adjacency list
    ...
    import scipy
    # Process sparse matrix coo matrix, convert to adjacency list
    if isinstance(M, scipy.sparse.coo.coo_matrix):
        row_list = M.row.tolist()
        col_list=M.col.tolist()
        node_set = set(row_list)
        dict = {}
        for item in node_set:
            dict.update({item: row_list.count(item)})
        k=0
        for i in set(row_list):
            adjline = [i]
            for j in range(dict[i]):
                adjline.append(col_list[k])
                k+=1
            adjlist.append(adjline)
